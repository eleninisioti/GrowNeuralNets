...
import evosax
from src.hypernet.model import make_model
...
def viz_trajectory(project_dir, trial, config):
    ...
    for eval_trial in range(10):
        ...
        eval_trial_rewards = onp.sum(data["reward"].tolist())
        rewards.append(eval_trial_rewards)

        print("Eval trial reward " + str(eval_trial_rewards))
        ...
        with open(saving_dir + "/traj_" + str(eval_trial) + "_" + str(eval_trial_rewards) + ".html", "w") as f:
            f.write(render)
    print("Evaluation for 10 trials gives " + str(onp.mean(rewards)))

    with open(saving_dir + "/eval.txt", "w" ) as f:
        f.write("Evaluation for 10 trials gives " + str(onp.mean(rewards)) + " with var " + str(onp.var(rewards)))
...
if __name__ == "__main__":
    ...
    top_dir = "scripts/networks_data/hypernet/"
    project_dirs = [top_dir + x for x in os.listdir(top_dir)]
    for project_dir in project_dirs:
        if "reacher" in project_dir:
        ...
        # plot developmental growth
            viz_project(project_dir, show_dev=False, show_evol=True)
