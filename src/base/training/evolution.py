...
from src.base.training.base import BaseMultiTrainer, BaseTrainer
...
class EvosaxTrainer(BaseTrainer):
	...
	def __init__(
		...
		if eval_reps > 1:
			def _eval_fn(p: Params, k: jr.PRNGKeyArray, tp: Optional[PyTree]=None):
				"""
				"""
				fit, info, policy_states = jax.vmap(task, in_axes=(None,0,None))(p, jr.split(k,eval_reps), tp)
				return jnp.mean(fit), info, policy_states
			self.task = _eval_fn
		else :
			self.task = task
	...
	def train_step(self, state: TrainState, key: jr.PRNGKeyArray, data: Optional[TaskParams]=None) -> Tuple[TrainState, Data]:
		...
		fitness, eval_data, interm_policies = self.eval(x, eval_key, data)
		f = self.fitness_shaper.apply(x, fitness)
		state = self.strategy.tell(x, f, state, self.es_params)
		
		return state, {"fitness": fitness, "best_indiv": jnp.argmax(fitness), "data": eval_data, "interm_policies": interm_policies}
