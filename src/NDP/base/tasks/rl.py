...
from AI_frameworks.ecobrax import envs
from AI_frameworks.ecobrax.envs import Env
...
class BraxTask(eqx.Module):
	...
	def __init__(
		...
		if isinstance(env, str):
			self.env = envs.get_environment(env, backend=backend)
		else:
			self.env = env
	...
	def __call__(
		...
		_, data = self.rollout(params, key)
	...
	def rollout(
		...
		params: Params, 
		key: jax.Array, 
		task_params: Optional[TaskParams]=None)->Tuple[State, dict]:
		...
		def env_step(carry, x):
			...
			action, policy_state = policy(state.env_state, state.policy_state, _key)
...
class StatefulPolicyWrapper(eqx.Module):
	...
	"""
	Wrapper adding a policy state to the signature call of a model without state
	"""
	#-------------------------------------------------------------------
