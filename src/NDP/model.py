...
import evosax as ex
import equinox as eqx
from src.NDP.base.models.gnn.layers import EMPNN, GNCA, CycleCountInjection, GAT, GraphMLP, MPNN, GraphTransformer, RMPNN, RecurrentAttentionNetwork
...
from src.NDP.base.models.gnn.ndp import (NDP, ENDP, SimpleNDP, RecurrentNDP, off_sigmoid)
...
class NodeModel(eqx.Module):
    ...
    def __call__(self, graph, key, counter):
        ...
        if self.inhibition:
            ...
            #h = jnp.where(counter == 1, h, oh)
            #h = jnp.where(counter == 2, h, oh)
...
def make_model(config, key):
    ...
    if config["model_config"]["inhibition"] == "all":
        inhibit_hidden = True
        inhibit_mitosis = True
        inhibit_weights = True
    elif config["model_config"]["inhibition"] == "hidden":
        inhibit_hidden = True
        inhibit_mitosis = False
        inhibit_weights = False
    elif config["model_config"]["inhibition"] == "hidden_and_mitosis":
        inhibit_hidden = True
        inhibit_mitosis = True
        inhibit_weights = False
    elif config["model_config"]["inhibition"] == "none":
    ...
    if config["model_config"]["intrinsic"]:
        node_features = node_features_learned + node_features_intrinsic
    else:
